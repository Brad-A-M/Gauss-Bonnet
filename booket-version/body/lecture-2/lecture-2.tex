...
In this section, we briefly introduce surfaces, polygonal meshes, describe a data structure to 
work with them, and introduce the Euler characteristic.
...
A familiar example is the unit circle with parameterization given by the map $C:[0,2\pi]\rightarrow \R^2$
$$C(t)=\left(\cos(t),\sin(t)\right).$$ Given a value of $t$ say $\frac{\pi}{2}$ the parameterization
gives a point, namely $(0,1)$.
A feature of parameterizations is that we include a velocity of the traversal.
...
Note that the curve is the image of the parameterization. Two different maps can result in the same curve but the speeds that we traverse the curve can be different.
...
\begin{enumerate}
 \item How many charts are needed to cover the two sphere?
