...
Two fundamental surfaces are the sphere and the plane. 
Although these surfaces appear quite different, they share many interesting properties and are closely related. In this section, we highlight some of their similarities and differences and introduce a map called stereographic projection.
 

\subsection{One Dimensional Analogs}

Before discussing the plane and the sphere, it is helpful to first develop our intuition in one dimension by considering the real line and the unit circle.

One way to study these objects is to regard them as subsets of $(\mathbb{R}^2)$. The real line can be written as
$$\R=\{(x,y)\in\mathbb{R}^2 : y=0\},$$

and the unit circle as

$$\Sp^1=\{(x,y)\in\mathbb{R}^2 : x^2+y^2=1\}.$$

This is an \emph{extrinsic} viewpoint: we study the objects from the outside by considering how they are embedded in a higher-dimensional space. Later, we will contrast this with the \emph{intrinsic} viewpoint, which focuses on the geometry experienced by an observer confined to the object itself.



Is the circle homeomorphic to the line?
The answer is no (Exercise 1).
...
and the two sphere is
$$\Sp^2=\{(x,y,z)\in\mathbb{R}^2 : x^2+y^2+z^2=1\}.$$
...
A first similarity between  $\R^2$ and $\Sp^2$ is that they are smooth, 
...
Is the plane homeomorphic to the two sphere?
Again, the answer is no, but stereographic projection maps all but one
point on the sphere continuously to the two sphere.
...
because we push tangent vectors from $S_1$ to $S_2$. See \figref{pushforward} for an example.
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=.4\textwidth]{background/pushforward}
...
This is not the case on the two sphere. On the sphere,
 we can relate the area of a simple polygon 
...
\begin{enumerate}
	\item Prove $\R$ is not homeomorphic to $\Sp^1.$
