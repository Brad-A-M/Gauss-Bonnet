...
to the Euler characteristic, a global property. More specifically, if we were travel over a surface and compute the curvature at each point, the sum of all curvatures is equal to the Euler characteristic. Conversely, if we know the Euler characteristic of a surface we are able to 
guarantee the existence of a point on the surface with a bounded curvature. 


In symbols, one version of the Gauss-Bonnet theorem is the following
...
The version given in \thmref{g-b-noboundary} is due to Pierre Bonnet
and Jacques Binet \cite{bonnet1848memoire}. A case can be made that the theorem
ought to be called the Gauss-Binet-Bonnet Theorem
...
In addition to the version for smooth surfaces given in \thmref{g-b-noboundary},
we highlight several discrete versions for triangulated surfaces. 
 
 
 
As for (2), several fundamentally different proofs exist.
We provide two distinct proofs. One using tangent vector fields on smooth surfaces and
another simple proof for triangulated polyhedra.
Commonly, the theorem is first proved for simply connected domains
with boundary, then triangulate more complicated surfaces and sum the contribution from each triangle.
However, this proof  lacks a geometric intuition that other proofs provide\cite{wu_historical_2008}.
...
For generalization (3), we will share versions for smooth
...
an infinite analog  in \secref{circle-packing}.
Two  celebrated generalizations to higher dimensions are the Chern-Gauss-Bonnet theorem \cite{chern_simple_1944}, which we
discuss in \secref{chern-gauss-bonnet}, and
the Atiyah–Singer index theorem  \cite{atiyah_index_1963}.




As for (4), applications, this work provides several. Seven are given in \cite{doc76}.
