...
Consider a simple polygon $P$.
An example is shown in \figref{polygon}.
...
The angle inside of $P$ is the \EMPH{interior} angle and the complement 
is the  \EMPH{exterior angle}. An example is shown in \figref{interior-exterior}.
The exterior angle at a vertex can be positive or negative, see \figref{exterior-angles}.
...
Let $P$ be a simple polygon in the plane with interior angles $\{\alpha_1,\alpha_2,\ldots,\alpha_n\}$
and exterior angles $\{\beta_1,\beta_2,\ldots,\beta_n\}$
$$\sum_{i=1}^n\beta_i=\sum_{i=1}^n(\pi-\alpha_i)=2\pi.$$
\end{theorem}
...
\begin{proof}
	...
	so for each $i$ we have $\beta_i=\pi-\alpha_i$.
